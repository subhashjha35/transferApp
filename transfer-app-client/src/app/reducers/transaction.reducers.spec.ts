import { createTransactionSuccess, listTransactions, listTransactionsFailed } from '../actions/transaction.actions';
  ...
  describe('listTransactionsSuccess', () => {
    it('should set transaction list', () => {
      expect(
        fromTransaction.reducers(
          initialState,
          listTransactionsSuccess({ transactions: [{ name: 'name1' }] as any })
        )
      ).toEqual({ ...initialState, isLoaded: true, isLoading: false, list: [{ name: 'name1' }] as any });
    });
  });
  ...
  describe('createTransactionSuccess', () => {
    it('should add one transaction to list', () => {
      expect(
        fromTransaction.reducers(
          initialState,
          createTransactionSuccess({ transaction: { name: 'name1' } as any })
        )
